...
VEC3::IsVertical(const VEC3 &rhs) const
{
	...
	return (-ML_OX_EPSILON < DotValue && DotValue < ML_OX_EPSILON);	// 誤差範囲内なら垂直と判定
}
...
VEC3::IsParallel(const VEC3 &rhs) const
{
	...
	return (-ML_OX_EPSILON < CrossValue && CrossValue < ML_OX_EPSILON);	// 誤差範囲内なら平行と判定
}
...
// テクスチャのUV座標の設定用の情報
//----------------------------------------------------------------------------------------------------
inline TEXTUREUV::TEXTUREUV(int nCntU, int nNumU, int nCntV, int nNumV)
{
	this->nCntU = nCntU;
	this->nNumU = nNumU;
	this->nCntV = nCntV;
	this->nNumV = nNumV;
	this->fIntervalU = 1.0f / this->nNumU;
	this->fIntervalV = 1.0f / this->nNumV;
}

//----------------------------------------------------------------------------------------------------
// テクスチャアニメーション情報
//----------------------------------------------------------------------------------------------------
inline _TEXTUREANIMEINFO::_TEXTUREANIMEINFO(int nFrame, int nFrameMax, int nIteration, bool bLoop, bool bIteration)
{
	this->nFrame = nFrame;
...
{
	this->m_nSize = ML_INT_UNSET;
	this->m_nCapacity = ML_INT_UNSET;
	this->m_pData = NULL;
}
...
// 変数宣言
	...
	if ((lTop != ML_FAILURE &&
		fseek(pFile, 0, SEEK_END) == ML_SUCCESS) != ML_TRUE)
	{
		return ML_FAILURE;
	}
	...
	if ((lSize != ML_FAILURE &&
		fseek(pFile, lTop, SEEK_SET) == ML_SUCCESS) != ML_TRUE)
	{
		return ML_FAILURE;
	}
...
CLoadInitSetingfromFile::SetingfromString(void(*ReadLine)(CONST_STRING cnpLine, CONST_STRING cnpEntryType, CONST_STRING cnpEntryData))
{
	...
	// バッファの最後を0に設定
	pBuffe[this->m_nuFileSize] = ML_CHAR_UNSET;
	...
	for (pLine = pBuffe; pLine < pBuffeEnd; pLine = pLineEnd + 1)
	{
		...
		// 行末から初期化
		pLineEnd[0] = ML_CHAR_UNSET;

		// 1行の最初が [ 最後が ] の時
		if (pLine[0] == '[' &&pLineEnd > pLine &&pLineEnd[-1] == ']')
		{
			// 1行の最後を初期化
			pLineEnd[-1] = ML_CHAR_UNSET;
			...
			if (pTypeEnd == ML_CHAR_UNSET ||
				pDataStart == ML_CHAR_UNSET)
			{
				continue;
			}
			// 一つ目の ] を初期化
			*pTypeEnd = ML_CHAR_UNSET;
			// 二つ目の [ を飛ばす
			pDataStart++;
...
CString::Init(void)
{
	...
	this->m_nStringLength = ML_INT_UNSET;	// 文字列の長さ
}
