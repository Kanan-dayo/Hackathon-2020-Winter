...
void CMylibrary::SetVatexPostion(D3DXVECTOR3 ** ppOut, ORIGINVERTEXTYPE type, D3DXVECTOR3 * pPos, D3DXVECTOR2 * pSize, float fRadius, float fRotation)
{
	...
	case ORIGINVERTEXTYPE_LOWERLEFT:
		*ppOut[0] = D3DXVECTOR3(pPos->x, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_LOWERCENTER:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x*ML_HALF_SIZE, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x*ML_HALF_SIZE, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x*ML_HALF_SIZE, pPos->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x*ML_HALF_SIZE, pPos->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_LOWERRIGHT:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x, pPos->y - pSize->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x, pPos->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_CENTERLEFT:
		*ppOut[0] = D3DXVECTOR3(pPos->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x, pPos->y + pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y + pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_CENTER:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x*ML_HALF_SIZE, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x*ML_HALF_SIZE, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x*ML_HALF_SIZE, pPos->y + pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x*ML_HALF_SIZE, pPos->y + pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_CENTERRIGHT:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x, pPos->y - pSize->y*ML_HALF_SIZE, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_UPPERLEFT:
		*ppOut[0] = D3DXVECTOR3(pPos->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_UPPERCENTER:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x + pSize->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_UPPERRIGHT:
		*ppOut[0] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[1] = D3DXVECTOR3(pPos->x, pPos->y, ML_FLOAT_UNSET);
		*ppOut[2] = D3DXVECTOR3(pPos->x - pSize->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		*ppOut[3] = D3DXVECTOR3(pPos->x, pPos->y + pSize->y, ML_FLOAT_UNSET);
		break;
	case ORIGINVERTEXTYPE_ROTATION:
		// 頂点の設定
		ppOut[0]->x = pPos->x + sinf(D3DX_PI*-ML_THREEQUARTERS_SIZE)*fRadius;
		ppOut[0]->y = pPos->y + cosf(D3DX_PI*-ML_THREEQUARTERS_SIZE)*fRadius;
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI*ML_THREEQUARTERS_SIZE)*fRadius;
		ppOut[1]->y = pPos->y + cosf(D3DX_PI*ML_THREEQUARTERS_SIZE)*fRadius;
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(D3DX_PI*-ML_ONEQUARTER_SIZE)*fRadius;
		ppOut[2]->y = pPos->y + cosf(D3DX_PI*-ML_ONEQUARTER_SIZE)*fRadius;
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(D3DX_PI*ML_ONEQUARTER_SIZE)*fRadius;
		ppOut[3]->y = pPos->y + cosf(D3DX_PI*ML_ONEQUARTER_SIZE)*fRadius;
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;
	case ORIGINVERTEXTYPE_ROTCENTER:
		ppOut[0]->x = pPos->x + sinf(D3DX_PI + fRotation)*pSize->y + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[0]->y = pPos->y + cosf(D3DX_PI + fRotation)*pSize->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI + fRotation)*pSize->y + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[1]->y = pPos->y + cosf(D3DX_PI + fRotation)*pSize->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[2]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[3]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;
	case ORIGINVERTEXTYPE_ROTCENTERLEFT:
		...
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI + fRotation)*pSize->y + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[1]->y = pPos->y + cosf(D3DX_PI + fRotation)*pSize->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y;
		ppOut[2]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y;
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[3]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;

	case ORIGINVERTEXTYPE_ROTLOWERCENTER:
		ppOut[0]->x = pPos->x + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x + sinf(D3DX_PI + fRotation)*(pSize->y*2.0f);
		ppOut[0]->y = pPos->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x + cosf(D3DX_PI + fRotation)*(pSize->y*2.0f);
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x + sinf(D3DX_PI + fRotation)*(pSize->y*2.0f);
		ppOut[1]->y = pPos->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x + cosf(D3DX_PI + fRotation)*(pSize->y*2.0f);
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[2]->y = pPos->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[3]->y = pPos->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;
	case ORIGINVERTEXTYPE_ROTCENTERRIGHT:
		ppOut[0]->x = pPos->x + sinf(D3DX_PI + fRotation)*pSize->y + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*(pSize->x)*2.0f;
		ppOut[0]->y = pPos->y + cosf(D3DX_PI + fRotation)*pSize->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*(pSize->x)*2.0f;
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI + fRotation)*pSize->y;
		ppOut[1]->y = pPos->y + cosf(D3DX_PI + fRotation)*pSize->y;
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[2]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*(pSize->x*2.0f);
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(ML_FLOAT_UNSET + fRotation)*pSize->y;
		ppOut[3]->y = pPos->y + cosf(ML_FLOAT_UNSET + fRotation)*pSize->y;
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;
	case ORIGINVERTEXTYPE_ROTUPPERCENTER:
		ppOut[0]->x = pPos->x + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[0]->y = pPos->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[0]->z = ML_FLOAT_UNSET;
		ppOut[1]->x = pPos->x + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[1]->y = pPos->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x;
		ppOut[1]->z = ML_FLOAT_UNSET;
		ppOut[2]->x = pPos->x + sinf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x + sinf(ML_FLOAT_UNSET + fRotation)*(pSize->y*2.0f);;
		ppOut[2]->y = pPos->y + cosf(D3DX_PI*-ML_HALF_SIZE + fRotation)*pSize->x + cosf(ML_FLOAT_UNSET + fRotation)*(pSize->y*2.0f);;
		ppOut[2]->z = ML_FLOAT_UNSET;
		ppOut[3]->x = pPos->x + sinf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x + sinf(ML_FLOAT_UNSET + fRotation)*(pSize->y*2.0f);;
		ppOut[3]->y = pPos->y + cosf(D3DX_PI*ML_HALF_SIZE + fRotation)*pSize->x + cosf(ML_FLOAT_UNSET + fRotation)*(pSize->y*2.0f);;
		ppOut[3]->z = ML_FLOAT_UNSET;
		break;
	}
}
...
//* [contents] 1.0f ～ 0.0f にクランプする
//* [in] 色
//* [out] true 丸めた : false 処理なし
bool CMylibrary::ClampCol(D3DXCOLOR & col)
{
	if (col.a <= 0.0f)
	{
		col.a = 0.0f;
...
void CMylibrary::SetD3DXCOLORfromREF_WithoutAlpha(COLORREF * SystemColor, D3DXCOLOR * col)
{
	...
	col->g = (BYTE)((*SystemColor) >> ML_D3DXCORG_BIT) / 255.0f;
	col->b = (BYTE)((*SystemColor) >> ML_D3DXCORB_BIT) / 255.0f;
}
...
void CMylibrary::SetD3DXCOLORfromREF(COLORREF * SystemColor, D3DXCOLOR * col)
{
	...
	col->g = (BYTE)((*SystemColor) >> ML_D3DXCORG_BIT) / 255.0f;
	col->b = (BYTE)((*SystemColor) >> ML_D3DXCORB_BIT) / 255.0f;
	col->a = (BYTE)((*SystemColor) >> ML_D3DXCORA_BIT) / 255.0f;
}
...
int CMylibrary::GetConvertStoI(STRING string)
{
	...
	if (bHit == false)
	{
		return ML_INT_NOELEM;
	}
	else if (nNum == ML_INT_UNSET)
	{
		return nNum;
	}
